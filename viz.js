...
function initVisualization() {
  ...
  d3.csv("/mice.csv").then(function(csvData) {
...
// Apply clip path to the activity and temperature lines
svg.append("path")
.datum(rolled)
.attr("class", "activity-line")
.attr("d", lineAct)
...
// Apply clip path to the background segments
svg.selectAll("rect.bg")
.attr("clip-path", "url(#clip)");
  ...
  .on("end", brushed); // Update the event handler to 'end'
...
svg.append("g")
  ...
  function brushed(event) {
    ...
    if (selection) {
      ...
      // Update the x-scale to match the brush selection
      const newX = d3.scaleLinear()
        .domain([x.invert(x0), x.invert(x1)])
        .range([0, width]);
  
      // Update the y-scale dynamically based on the new x-range
      const newYAct = d3.scaleLinear()
        .domain([0, d3.max(rolled.filter(d => x(d.min) >= x0 && x(d.min) <= x1), d => d.avgAct)])
        .nice()
        .range([height, 0]);
  
      const newYTemp = d3.scaleLinear()
        .domain(d3.extent(rolled.filter(d => x(d.min) >= x0 && x(d.min) <= x1), d => d.avgTemp))
        .nice()
        .range([height, 0]);
  
      // Update the lines based on the new x-scale and y-scale
      svg.selectAll("path.activity-line")
        .transition().duration(200)
        .attr("d", lineAct.x(d => newX(d.min)).y(d => newYAct(d.avgAct)));
  
      svg.selectAll("path.temperature-line")
        .transition().duration(200)
        .attr("d", lineTemp.x(d => newX(d.min)).y(d => newYTemp(d.avgTemp)));
      ...
      svg.select(".y-axis.left")
        .transition().duration(200)
        .call(d3.axisLeft(newYAct));
  
      svg.select(".y-axis.right")
        .transition().duration(200)
        .call(d3.axisRight(newYTemp));
